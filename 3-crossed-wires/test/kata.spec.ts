...
import { Wire, Coordinates, CrossedWire } from '../src/coordinates';

describe('Grid', () => {
  it('it has 2 paths with no wires when created', () => {
    const grid = new Grid();
    const empytPath: Wire[] = [];

    expect(grid.firstPath.value).toStrictEqual(empytPath);
    expect(grid.secondPath.value).toStrictEqual(empytPath);
  });

  it('has 2 wire paths that starts from (0,0)', () => {
    const startingPoint = new Wire(0, 0);
    const grid = new Grid();

    expect(grid.firstPath.currentPosition).toStrictEqual(startingPoint);
    expect(grid.secondPath.currentPosition).toStrictEqual(startingPoint);
  });

  it('fills each path with wires depending on the given instruction', () => {
    const grid = new Grid();
    const expectedFirstPath: Wire[] = [
      new Wire(0, 1),
      new Wire(0, 2),
      new Wire(0, 3),
    ];
    const expectedSecondPath: Wire[] = [new Wire(-1, 0), new Wire(-2, 0)];

    grid.wirePaths('U3', 'L2');

    expect(grid.firstPath.value).toStrictEqual(expectedFirstPath);
    expect(grid.secondPath.value).toStrictEqual(expectedSecondPath);
  });

  it('fills paths depending on instructions, the next instruction begins at the end of the previous one', () => {
    const grid = new Grid();
    const expectedFirstPath: Wire[] = [
      new Wire(0, 1),
      new Wire(0, 2),
  ...
  it('can wire up, right, left and down', () => {
    ...
    grid.wirePaths('U1,R1,D2,L1', 'L1');

    expect(grid.firstPath.value).toStrictEqual(expectedPath);
  });

  it('adds a crossed wire in the position where the second wire finds the first one', () => {
    const grid = new Grid();
    const expectedCrossedWires: CrossedWire[] = [new CrossedWire(1, 1)];

    grid.wirePaths('U1,R1', 'R1,U1');

    expect(grid.crossedWires).toStrictEqual(expectedCrossedWires);
  });

  it('adds crossed wires in common positions except on 0,0', () => {
    const grid = new Grid();
    const expectedCrossedWires: CrossedWire[] = [
      new CrossedWire(0, 1),
      new CrossedWire(1, 0),
      new CrossedWire(-1, 0),
    ];

    grid.wirePaths('U1,R1,D1,L2', 'U2,R2,D2,L4');

    expect(grid.crossedWires).toStrictEqual(expectedCrossedWires);
  });

  it('gets the manhattan distance of the closest crossed wire', () => {
    const gridOne = new Grid();
    const gridTwo = new Grid();
    ...
    expect(gridOne.manhattanDistanceToClosestCross).toBe(159);
    expect(gridTwo.manhattanDistanceToClosestCross).toBe(135);
  });
  ...
  it('answers the question', () => {
    ...
    console.log(grid.manhattanDistanceToClosestCross);
  });
});
